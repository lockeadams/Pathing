...
public class Path extends ArrayList<Point2D> {
	...
	private ArrayList<Point2D> waypoints, injectedPoints, smoothPoints;
	private Point2D normalizedP1, normalizedP2;
	private Vector normalized, spaced;
	...
	public ArrayList<Point2D> getInjected() {
		if(injectedPoints == null) System.out.println("error: please inject points in the path using injectPoints(double spacing).");
	...
	public void smooth(int radius) {
		for(int c = 0; c < waypoints.size() - 2; c++) {
			if(injectedPoints == null) injectPoints(0.5);
	...
	public ArrayList<Point2D> getSmoothed() {
		if(smoothPoints == null) System.out.println("Error: please smooth the path using smooth(int radius).");
		return smoothPoints;
	}
